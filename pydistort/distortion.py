...
class Distortion(Transform):
    ...
    # =============================================
    # I / O Methods for Distortion Class
    # =============================================
    def to_file(self, filename: str, description: Optional[str] = None) -> None:
        r"""
        Save the distortion model to a file.
        ...
        .. code-block:: text
            ...
            package : pydistort
            url : https://github.com/Artezaru/pydistort
        ...
        ----------
        filename : str
            The name of the file to save the distortion model.

        description : Optional[str], optional
            A description of the distortion model, by default None.
        """
        with open(filename, 'w') as f:
            f.write("# DISTORTION MODEL FILE\n")
            f.write(f"description : {description}\n" if description else "description : None\n")
            f.write(f"class : {self.__class__.__name__}\n")
            f.write("\n")
            f.write("# FILE USAGE INFORMATION\n")
            f.write("package : pydistort\n")
            f.write("url : https://github.com/Artezaru/pydistort\n")
            f.write("\n")
            f.write("# PARAMETERS SECTION (START)\n")
            if self.parameters is not None:
                for param in self.parameters.flatten():
                    f.write(f"{param}\n")
            f.write("# PARAMETERS SECTION (END)\n")

    
    @classmethod
    def from_file(cls, filename: str) -> 'Distortion':
        r"""
        Load the distortion model from a file.
        ...
        ----------
        filename : str
            The name of the file to load the distortion model from.
        
        Returns
        -------
        Distortion
            An instance of the distortion model loaded from the file.
        """
        with open(filename, 'r') as f:
            lines = f.readlines()

        # Search for the class name and parameters section
        class_name = None
        parameters = []
        ...
        for line in lines:
            ...
            elif line.startswith("# PARAMETERS SECTION (START)"):
                in_parameters_section = True
            elif line.startswith("# PARAMETERS SECTION (END)"):
                in_parameters_section = False
        ...
        if class_name != cls.__name__:
            print(f"Warning: Expected class name '{cls.__name__}', but found '{class_name}' in the file.")

        # Create an instance of the class
        instance = cls()
        instance.parameters = numpy.array(parameters) if parameters else None
        return instance

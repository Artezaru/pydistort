...
noise_amplitude = numpy.linspace(0.0, 10.0, 20)
Nmean = 10
distortion_list = []
std_distortion_list = []

for index, noise in enumerate(noise_amplitude):

    print(f"Optimizing distortion parameters for noise amplitude {noise:.2f} pixels ({index + 1}/{len(noise_amplitude)})...")
    ...
    for _ in range(Nmean):
        ...
        # Add noise to the real displacement field
        radial_noise = noise # * numpy.random.rand(real_displacement_field.shape[0])
        angular_noise = 2 * numpy.pi * numpy.random.rand(real_displacement_field.shape[0])
        displacement_noise = numpy.column_stack((radial_noise * numpy.cos(angular_noise), radial_noise * numpy.sin(angular_noise)))

        noisy_real_displacement_field = real_displacement_field + displacement_noise
        ...
        # Compute the associated noisy distorted points
        noisy_real_distorted_points = pixel_points + noisy_real_displacement_field

        optimized_parameters = optimize_iter_distortion.optimize_parameters(
            input_points=pixel_points[mask_optimisation, :],
            output_points=noisy_real_distorted_points[mask_optimisation, :],
            guess=None, # Use the distortion parameters as initial guess (here zero because the object is initialized with default parameters)
            max_iter=mas_iter,
            eps=eps,
            reg_factor=reg_factor, # Regularization factor (0.0 means no regularization)
...
if DISPLAY_INTERMEDIATE:
    ...
    print(f"Displaying the distortion parameters for noise index {noise_index} : {noise_amplitude[noise_index]} pixels")
    ...
    ax1.set_title("Optimized Distortion Parameters (Cx) for Noise Amplitude = {:.2f} pixels".format(noise_amplitude[noise_index]))
    ...
    ax2.set_title("Optimized Distortion Parameters (Cy) for Noise Amplitude = {:.2f} pixels".format(noise_amplitude[noise_index]))
...
fig_optimized_parameters.suptitle("Distortion Parameters for various displacements noise amplitudes \n U = U + noise * cos(theta), V = V + noise * sin(theta)", fontsize=16, fontweight='bold')
...
for i, (noise_distortion, std_noise_distortion) in enumerate(zip(distortion_list, std_distortion_list)):
    ax1.bar(x + i * x_width / len(distortion_list), noise_distortion.parameters_x, width=x_width / len(distortion_list), label=f'(Cx) - Noise {noise_amplitude[i]:.2f} pixels', color=colormap(i / len(distortion_list)))
...
for i, (noise_distortion, std_noise_distortion) in enumerate(zip(distortion_list, std_distortion_list)):
    ax2.bar(x + i * x_width / len(distortion_list), noise_distortion.parameters_y, width=x_width / len(distortion_list), label=f'(Cy) - Noise {noise_amplitude[i]:.2f} pixels', color=colormap(i / len(distortion_list)))
...
for i, (noise_distortion, std_noise_distortion) in enumerate(zip(distortion_list, std_distortion_list)):
    ax3.bar(x + i * x_width / len(distortion_list), noise_distortion.parameters_x, width=x_width / len(distortion_list), label=f'(Cx) - Noise {noise_amplitude[i]:.2f} pixels', color=colormap(i / len(distortion_list)))
...
for i, (noise_distortion, std_noise_distortion) in enumerate(zip(distortion_list, std_distortion_list)):
    ax4.bar(x + i * x_width / len(distortion_list), noise_distortion.parameters_y, width=x_width / len(distortion_list), label=f'(Cy) - Noise {noise_amplitude[i]:.2f} pixels', color=colormap(i / len(distortion_list)))

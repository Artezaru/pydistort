import numpy as np
import pytest
from pydistort import Cv2Distortion, undistort_points, Cv2Intrinsic, Cv2Extrinsic
import cv2
import time
import csv
...
def test_pydistort_undistort_vs_opencv(Nparams, mode):
    ...
    Pobject = Cv2Intrinsic(intrinsic_matrix=P)

    # 2D points
    points = setup.ORI_IMAGE_POINTS()

    # Undistort with your method
    undistorted_points = undistort_points(points, intrinsic=intrinsic, distortion=distortion, R=Robject, P=Pobject)
...
def test_pydistort_undistort_vs_opencv_timer(Nparams, mode):
    ...
    if setup.TIMER():
        ...
        for Nparams in Nparams_list:
            ...
            R = np.array([0.01, 0.02, 0.03])  # small rotation
            Robject = Cv2Extrinsic(rvec=R, tvec=np.zeros((3,)))

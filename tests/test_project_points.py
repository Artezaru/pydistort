import numpy as np
import pytest
from pydistort import Cv2Distortion, Cv2Extrinsic, Cv2Intrinsic, project_points, ZernikeDistortion
import cv2
import time
import csv
...
def test_pydistort_project_vs_opencv(Nparams, mode):
    ...
    result = project_points(points_3d, extrinsic, distortion, intrinsic, dx=True, dp=True)
    ...
    np.testing.assert_allclose(result.jacobian_dextrinsic, jacobian_dp_cv[:, :, 0:6], rtol=1e-5, atol=1e-8)
    np.testing.assert_allclose(result.jacobian_dintrinsic, jacobian_dp_cv[:, :, 6:10], rtol=1e-5, atol=1e-8)
    np.testing.assert_allclose(result.jacobian_ddistortion, jacobian_dp_cv[:, :, 10:], rtol=1e-5, atol=1e-8)
...
def test_pydistort_project_vs_opencv_timer(Nparams, mode):
    ...
    if setup.TIMER():
        ...
        for Nparams in Nparams_list:
            ...
            result = project_points(points, extrinsic, distortion, intrinsic, dx=True, dp=True)
            elapsed_time = time.perf_counter() - start_time
            pydistort_alljac_times.append(elapsed_time)

            # Projection (analytic)
            start_time = time.perf_counter()
            result = project_points(points, extrinsic, distortion, intrinsic, dx=False, dp=True)
            elapsed_time = time.perf_counter() - start_time
            pydistort_times.append(elapsed_time)

            # Projection (analytic)
            start_time = time.perf_counter()
            result = project_points(points, extrinsic, distortion, intrinsic, dx=False, dp=False)
            elapsed_time = time.perf_counter() - start_time
...
def test_pydistort_project_zernike():
    ...
    if setup.TIMER():
        ...
        for Nzer in Nzer_list:
            ...
            start_time = time.perf_counter()
            result = project_points(points, extrinsic, distortion, intrinsic, dx=True, dp=False)
            elapsed_time = time.perf_counter() - start_time
